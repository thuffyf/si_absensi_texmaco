    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div class="flex flex-wrap gap-2">
            <form method="GET" action="{{ route('absensi.index') }}" class="flex gap-2">
                <input type="text" name="search" value="{{ request('search') }}" class="w-48 rounded-xl border border-slate-300 px-4 py-2 text-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500" placeholder="Cari siswa..." />
                                        ...
                                        <div class="w-full max-w-md rounded-3xl bg-white p-6 shadow-xl whitespace-normal text-left">
                                            ...
                                            <form method="POST" action="{{ route('absensi.update', $record) }}" class="space-y-4">
                                                ...
                                                <div>
                                                    ...
                                                    <select name="status" class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500" required>
                                                        <option value="hadir" @selected($record->status === 'hadir')>Hadir</option>
                                                        <option value="izin" @selected($record->status === 'izin')>Izin</option>
                                                        <option value="sakit" @selected($record->status === 'sakit')>Sakit</option>
                                                        <option value="alpa" @selected($record->status === 'alpa')>Alpa</option>
                                                    </select>
                                                </div>
                                                <div>
